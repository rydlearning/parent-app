...
export default function RegResumptionModal({ handleNext, setChildInfo, closeModalOnOutsideClick, closeRegTab }: Props) {
    ...
    const userInfoTarget: any = useSelector((state: RootState) => state.user.resumeChildReg);
    ...
    useEffect(() => {
        if(userInfoTarget?.age>=7){
            ...
            //filer cohort
            if(userInfoTarget.age<17){
                //filter cohort
                setCohortArr(cohortKeep.filter((c: any) => Number(c.isVisible) !== SPECIAL_COHORT_NUM))
            }else{
                //filter cohort
                setCohortArr(cohortKeep.filter((c: any) => Number(c.isVisible) === SPECIAL_COHORT_NUM))
            }

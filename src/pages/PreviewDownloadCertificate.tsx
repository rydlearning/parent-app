...
const PreviewDownloadCertificate = () => {
  ...
  const getProgram = async () => {
    setLoading(true);
    try {
      const response = await userService.getProgramsCertificateData(Number(id));
      setLoading(false);
      if (!response.status) {
        return;
      }
      ...
      const CertificateData = {
        ...
        date: FormatEndDate(data.endClassDate),
      };
...
export default PreviewDownloadCertificate;
